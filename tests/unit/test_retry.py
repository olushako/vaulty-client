...
from vaulty.retry import retry_with_backoff, RetryConfig
...
async def test_retry_retries_on_5xx():
    ...
    func = AsyncMock(side_effect=[
        VaultyAPIError("Server error", 500),
        "success"
    ])
...
async def test_retry_respects_max_delay():
    ...
    config = RetryConfig(max_retries=2, initial_delay=50.0, backoff_factor=2.0, max_delay=60.0, jitter=False)
